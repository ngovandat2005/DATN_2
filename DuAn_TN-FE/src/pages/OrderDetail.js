...
const OrderDetailPage = () => {
  ...
  useEffect(() => {
    const fetchOrderInfo = async () => {
      try {
        const res = await fetch(`http://localhost:8080/api/donhang/chi-tiet/${id}`);
        if (!res.ok) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (isInventoryError) {
          ...
          // ✅ SỬA: Hiß╗ân thị các lß╗ùi khác vß╗¢i thông tin chi tiết
          Swal.fire({
            icon: 'error',
            ...
            html: `
              <div style="text-align: left;">
                ...
                <div style="background: #f8d7da; border: 1px solid #f5c6cb; border-radius: 8px; padding: 16px; margin-top: 16px;">
                  <h4 style="margin: 0 0 12px 0; color: #721c24;">🔍 Chi tiết lß╗ùi:</h4>
                  <pre style="background: #f8f9fa; padding: 12px; border-radius: 4px; font-size: 12px; color: #495057; overflow-x: auto; white-space: pre-wrap;">${errorMessage}</pre>
                </div>
                <div style="background: #fff5f5; border: 1px solid #fed7d7; border-radius: 8px; padding: 16px; margin-top: 16px;">
                  <h4 style="margin: 0 0 12px 0; color: #c53030;">≡ƒöº Khắc phß╗Ñc:</h4>
                  <ul style="margin: 0; padding-left: 20px; color: #c53030;">
                    <li>Kiß╗âm tra log backend đß╗â xem chi tiết lß╗ùi</li>
                    <li>Thß╗¡ lại sau vài gi├óy</li>
                    <li>Li├¬n hß╗ç quản trị vi├¬n nß║┐u vấn đß╗ü vß║½n tiß║┐p tß╗Ñc</li>
                  </ul>
                </div>
              </div>
            `,
        ...
        html: `
          <div style="text-align: left;">
            <p style="color: #27ae60; font-weight: 600; margin-bottom: 16px;">
              Đơn hàng #${id} đã được xác nhận thành công!
            </p>
          ...
          Swal.fire({
            ...
            html: `
              <div style="text-align: left;">
                ...
                <div style="background: #f8d7da; border: 1px solid #f5c6cb; border-radius: 8px; padding: 16px; margin-top: 16px;">
                  <h4 style="margin: 0 0 12px 0; color: #721c24;">🛠️ Khắc phục:</h4>
    ...
    const { value: formValues } = await Swal.fire({
      ...
      html: `
        <div style="text-align: left; margin-bottom: 20px;">
          ...
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason6" name="reason" value="Muốn đối phương thức thanh toán" style="margin-right: 8px;">
            <label for="reason6">Muốn đối phương thức thanh toán</label>
          </div>
          <div style="margin-bottom: 10px;">
  ...
  // ✅ Cập nhật địa chỉ giao hàng và tính lại phí ship
  ...
  const handleOpenEditAddressForm = async () => {
    // Kiß╗âm tra trạng thái đơn hàng
    if (orderInfo && orderInfo.trangThai !== 0) {
      Swal.fire({
        icon: 'warning',
          ...
          if (wardCode) {
            ...
            try {
              ...
              const res = await fetch(`http://localhost:8080/api/ghn/calculate-fee`, {
                ...
                body: JSON.stringify({
                  fromDistrict: 1484,                    // ⬅️ Ba Đình, Hà Nội (cố định như Payment.js)
                  toDistrict: parseInt(districtSelect.value), // ⬅️ Quận đích
                  toWardCode: wardCode,                   // ⬅️ Format cũ như Payment.js
                  weight: 500,
                  length: 20,
                  width: 20,
                  height: 10
  ...
  const handleConfirmCapNhatTrangThai = async (trangThaiMoi, actionName) => {
    const result = await Swal.fire({
      ...
      text: `Bạn có chắc chắn muốn ${actionName} đơn hàng #${id} không?`,
      icon: 'question',
      showCancelButton: true,
  ...
  const handleGiaoKhongThanhCong = async () => {
    ...
    const confirmResult = await Swal.fire({
      ...
      text: `Bạn có chắc chắn muốn đánh dấu đơn hàng #${id} là giao hàng không thành công không?`,
      icon: 'warning',
      showCancelButton: true,
    ...
    // ✅ TH├èM: Form nhập lý do giao hàng không thành công
    const { value: formValues } = await Swal.fire({
      title: '≡ƒÜ½ Giao hàng không thành công',
      text: 'Đ╞ín hàng này đang ở trạng thái "Đang giao". Vui lòng chọn lý do giao hàng không thành công:',
      html: `
        <div style="text-align: left; margin-bottom: 20px;">
          <div style="margin-bottom: 10px;">
          ...
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason2" name="reason" value="Khách tß╗½ chối nhận hàng" style="margin-right: 8px;">
            <label for="reason2">Khách tß╗½ chối nhận hàng</label>
          </div>
          <div style="margin-bottom: 10px;">
          ...
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason4" name="reason" value="L├╜ do khác" style="margin-right: 8px;">
            <label for="reason4">L├╜ do khác</label>
          </div>
          <div id="customReasonDiv" style="display: none; margin-top: 10px;">
            <textarea id="customReason" placeholder="Nhß║¡p lý do khác..." style="width: 100%; height: 80px; padding: 8px; border: 1px solid #ddd; border-radius: 4px; resize: vertical;"></textarea>
          </div>
        </div>
      `,
      showCancelButton: true,
      ...
      try {
        ...
        Swal.fire({
          ...
          html: `
            <div style="text-align: left;">
              <p style="color: #27ae60; font-weight: 600; margin-bottom: 16px;">
                Đơn hàng #${id} đã được đánh dấu là "Giao hàng không thành công"!
              </p>
              <div style="background: #d4edda; border: 1px solid #c3e6cb; border-radius: 8px; padding: 16px; margin-top: 16px;">
                <h4 style="margin: 0 0 12px 0; color: #155724;">📋 Thông tin cập nhật:</h4>
                <ul style="margin: 0; padding-left: 20px; color: #155724;">
        ...
        Swal.fire({
          ...
          html: `
            <div style="text-align: left;">
              ...
              <div style="background: #fff5f5; border: 1px solid #fed7d7; border-radius: 8px; padding: 16px; margin-top: 16px;">
                <h4 style="margin: 0 0 12px 0; color: #c53030;">🛠️ Khắc phục:</h4>
                <ul style="margin: 0; padding-left: 20px; color: #c53030;">
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1100, margin: '0 auto', background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px rgba(25,118,210,0.08)' }}>
      <button onClick={() => navigate(-1)} style={{ marginBottom: 24, background: '#e3f0ff', color: '#1976d2', border: 'none', borderRadius: 6, padding: '8px 24px', fontWeight: 600, cursor: 'pointer' }}>⬅️ Quay lại</button>
      {/* Stepper trạng thái trong div ri├¬ng */}
      {orderInfo && (
        <div style={{
          border: '1px solid #e3e8ee',
          borderRadius: 12,
