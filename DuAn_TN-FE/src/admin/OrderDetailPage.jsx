...
const OrderDetail = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const orderResponse = await axios.get(`http://localhost:8080/api/donhang/chi-tiet/${orderId}`);
      ...
      const productsResponse = await axios.get(`http://localhost:8080/api/donhangchitiet/don-hang/${orderId}`);
  ...
  const handleHuyDon = async () => {
      ...
      html: `
        <div style="text-align: left; margin-bottom: 20px;">
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason1" name="reason" value="Hết hàng" style="margin-right: 8px;">
            <label for="reason1">Hết hàng</label>
          </div>
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason2" name="reason" value="Khách hàng yêu cầu hủy" style="margin-right: 8px;">
            <label for="reason2">Khách hàng yêu cầu hủy</label>
          </div>
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason3" name="reason" value="Địa chỉ giao hàng không hợp lệ" style="margin-right: 8px;">
            <label for="reason3">Địa chỉ giao hàng không hợp lệ</label>
          </div>
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason4" name="reason" value="Không liên lạc được với khách hàng" style="margin-right: 8px;">
            <label for="reason4">Không liên lạc được với khách hàng</label>
          </div>
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason5" name="reason" value="Sản phẩm bị lỗi/hư hỏng" style="margin-right: 8px;">
            <label for="reason5">Sản phẩm bị lỗi/hư hỏng</label>
          </div>
          <div style="margin-bottom: 10px;">
            <input type="radio" id="reason6" name="reason" value="Trùng lặp đơn hàng" style="margin-right: 8px;">
            <label for="reason6">Trùng lặp đơn hàng</label>
          </div>
          <div style="margin-bottom: 10px;">
    ...
    if (formValues) {
      try {
        const response = await axios.put(`http://localhost:8080/api/donhang/huy/${orderId}`, {
          ghiChu: formValues
        });
  ...
  const handleUpdateStatus = async (nextStatus) => {
    const statusObj = TRANG_THAI.find(t => t.value === nextStatus);
    const confirmResult = await Swal.fire({
      title: 'Xác nhận?',
      text: `Bạn có chắc chắn muốn chuyển đơn hàng sang trạng thái "${statusObj.label}"?`,
      icon: 'question',
      showCancelButton: true,
    ...
    if (confirmResult.isConfirmed) {
      try {
        const response = await axios.put(`http://localhost:8080/api/don-hang/${orderId}/trang-thai?value=${nextStatus}`);
        if (response.status === 200) {
          Swal.fire({
            icon: 'success',
            title: 'Thành công!',
            text: `Đã chuyển trạng thái sang "${statusObj.label}"`,
            timer: 1500,
        ...
        Swal.fire({
          icon: 'error',
          title: 'Lỗi!',
          text: 'Không thể cập nhật trạng thái: ' + (err.response?.data || err.message),
        });
      }
    }
  };
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1100, margin: '0 auto', background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px rgba(25,118,210,0.08)' }}>
      ...
      <div style={{
        ...
        {/* Nhóm các nút điều hướng trạng thái */}
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 16 }}>
          {/* Nút Xác nhận đơn (0 -> 1) */}
          {order.trangThai === 0 && (
            <button
              style={{
                padding: '10px 24px',
        ...
        <div style={{ fontSize: 13, color: '#666', marginTop: 12, fontStyle: 'italic' }}>
          💡 Tip: Bạn có thể thay đổi trạng thái theo trình tự quy trình bán hàng.
        </div>
